...
- Поддержать выбор задач только из одного настроенного проекта Linear и только из настраиваемых state ID для маршрутов `ready to propose`, `ready to code` и `ready to archive`.
...
- Заложить в конфиг отдельные target state ID для review-этапов `Need Proposal Review`, `Need Code Review`, `Need Archive Review`, чтобы `CoreOrch` мог явно переводить задачу туда после завершения `propose/code/archive` executor'ом.
...
- Расширение `internal/config` и `.env.example` новыми runtime-параметрами Linear и state mapping, включая review target states.
