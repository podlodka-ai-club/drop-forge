...
func TestRunnerHappyPathResolvesPRBranchCommitsAndPushes(t *testing.T) {
	...
	assertCommand(t, fake.commands[5], "git", []string{"commit", "-m", "Archive: ENG-1: Archive feature"}, cloneDir)
...
func TestRunnerUsesProvidedBranchWithoutGitHubLookupAndCleansTemp(t *testing.T) {
	...
	if removedPath != "/tmp/orchv3-archive" {
...
func TestRunnerRejectsMissingBranchSourceBeforeSideEffects(t *testing.T) {
	...
	err := runner.Run(context.Background(), ArchiveInput{Title: "Task", AgentPrompt: "Context"})
...
func TestRunnerReturnsConfigValidationBeforeSideEffects(t *testing.T) {
	...
	err := runner.Run(context.Background(), ArchiveInput{Title: "Task", AgentPrompt: "Context", BranchName: "branch"})
...
func TestCodexPromptUsesArchiveSkill(t *testing.T) {
	prompt := buildCodexPrompt("Task context")
	if !strings.Contains(prompt, "openspec-archive-change") || !strings.Contains(prompt, "Task context") {
...
func runWithResponses(t *testing.T, responses []fakeResponse, agent *fakeAgentExecutor) error {
	...
	return runner.Run(context.Background(), ArchiveInput{
		Title:       "Archive task",
		AgentPrompt: "Task context",
		BranchName:  "feature/task",
	})
