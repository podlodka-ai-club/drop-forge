...
CLI запускается без аргументов и стартует постоянный orchestration monitor. `stdout` остается пустым. Логи monitor-а и шагов `temp`, `git`, `codex` и `github` печатаются в stderr или настроенный log sink. Модуль `codex` относится к текущей реализации agent executor. В этом же monitor-е задачи из `Ready to Code` обрабатывает Apply runner, который использует те же `PROPOSAL_*` настройки репозитория и CLI-путей, но пушит изменения в существующую ветку PR без создания нового pull request.
...
Доступные переменные перечислены в `.env.example` без значений. Для запуска proposal/apply runner обязательно указать `PROPOSAL_REPOSITORY_URL`; остальные поля имеют безопасные значения по умолчанию в коде и могут быть переопределены через environment. `PROPOSAL_POLL_INTERVAL` задает паузу между проходами orchestration monitor. `PROPOSAL_CODEX_PATH` остается путем к Codex CLI для текущей реализации agent executor.
