...
const defaultTempPattern = "orchv3-apply-*"
...
func (input ApplyInput) validate() error {
	if strings.TrimSpace(input.Title) == "" {
		return errors.New("apply input title must not be empty")
	}
	if strings.TrimSpace(input.AgentPrompt) == "" {
		return errors.New("apply input agent prompt must not be empty")
	}
	if strings.TrimSpace(input.PRURL) == "" && strings.TrimSpace(input.BranchName) == "" {
		return errors.New("apply input branch source must include pr url or branch name")
	}

	return nil
}

func (runner *Runner) Run(ctx context.Context, input ApplyInput) (err error) {
	if err := input.validate(); err != nil {
		return err
	}
	if err := validateConfig(runner.Config); err != nil {
		return fmt.Errorf("validate apply runner config: %w", err)
	}
	...
	defer func() {
		if err != nil {
			logger.Errorf("applyrunner", "workflow failed: %v", err)
		}
	}()
	...
	if _, err := runner.agentExecutor(command).Run(ctx, AgentExecutionInput{
		...
		return fmt.Errorf("agent apply: %w", err)
	}
...
func BuildCommitMessage(identifier string, title string) string {
	...
	return proposalrunner.BuildPRTitle("Apply:", displayName)
}

func (runner *Runner) resolveBranch(ctx context.Context, command commandrunner.Runner, cloneDir string, input ApplyInput, stdout io.Writer, stderr io.Writer) (string, error) {
