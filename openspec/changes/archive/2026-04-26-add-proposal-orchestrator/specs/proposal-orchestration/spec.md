...
The system SHALL attach the proposal PR URL to the task and then move the task to `LINEAR_STATE_NEED_PROPOSAL_REVIEW_ID` after the proposal runner succeeds.

#### Scenario: Proposal task reaches review state

- **WHEN** the proposal runner returns a PR URL for a ready-to-propose task
...
#### Scenario: PR attachment happens before state transition

- **WHEN** a ready-to-propose task is successfully processed
- **THEN** the orchestration stage attaches the PR URL before moving the task to the proposal review state

### Requirement: Proposal orchestration preserves task state on failure

The system SHALL return contextual errors and avoid moving a task to proposal review when proposal execution or PR attachment fails.

#### Scenario: Proposal runner fails

- **WHEN** the proposal runner returns an error for a ready-to-propose task
